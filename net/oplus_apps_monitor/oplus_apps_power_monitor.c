...
extern int app_monitor_dl_ctl_msg_handle(struct nlmsghdr *nlh);
extern int app_monitor_dl_report_msg_handle(struct nlmsghdr *nlh);
...
static void report_timer_init(void)
{
	...
	timer_setup(&apps_monitor_report_timer, apps_monitor_report_timer_function, 0);
}
