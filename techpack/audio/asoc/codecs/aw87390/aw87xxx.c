...
static const char *const aw87xxx_volume_boost[] = {"Voltage8", "Voltage7"};
#endif
...
#ifdef OPLUS_AUDIO_PA_BOOST_VOLTAGE
int aw87xxx_set_boost_voltage(struct aw87xxx *aw87xxx, int level)
{
	int ret = -1;
	struct aw_voltags_desc *vol_desc = NULL;

		AW_DEV_LOGI(aw87xxx->dev, "enter, set dev_index = %d, level = %d", aw87xxx->dev_index, level);
		...
		switch(level) {
			...
			case 1:
				...
				break;
			default:
				AW_DEV_LOGE(aw87xxx->dev, "unsupport status: %d", level);
				break;
		}
...
int aw87xxx_get_boost_voltage(struct aw87xxx *aw87xxx, int *level)
{
	int ret = -1;
	...
	struct aw_voltags_desc *vol_desc = NULL;
		...
		if (ret < 0) {
			AW_DEV_LOGE(aw87xxx->dev, "dev_index: %d read reg failed 0x%02x", aw87xxx->dev_index, vol_desc->addr);
			return ret;
		}
		if (reg_voltags_val ==  vol_desc->vol_max) {
			...
			*level = 1;
			...
			AW_DEV_LOGE(aw87xxx->dev, "dev_index: %d undefined reg val 0x%02x = 0x%02x",
				aw87xxx->dev_index, vol_desc->addr, reg_voltags_val);
			return -EINVAL;
		}
		AW_DEV_LOGI(aw87xxx->dev, "Get dev_index = %d, level = %d", aw87xxx->dev_index, *level);
		return 0;

}
EXPORT_SYMBOL(aw87xxx_get_boost_voltage);

static int aw87xxx_volume_boost_info(struct snd_kcontrol *kcontrol,
                        struct snd_ctl_elem_info *uinfo)
{
	int count;
	...
	uinfo->type = SNDRV_CTL_ELEM_TYPE_ENUMERATED;
	uinfo->count = 1;
	count = ARRAY_SIZE(aw87xxx_volume_boost);
	...
	strlcpy(uinfo->value.enumerated.name,
		aw87xxx_volume_boost[uinfo->value.enumerated.item],
		strlen(aw87xxx_volume_boost[uinfo->value.enumerated.item]) + 1);

	return 0;
}

static int aw87xxx_volume_boost_put(struct snd_kcontrol *kcontrol,
                        struct snd_ctl_elem_value *ucontrol)
{
	int ctrl_value = ucontrol->value.integer.value[0];
	struct aw87xxx *aw87xxx = (struct aw87xxx *)kcontrol->private_value;
	int ret = -1;

	/* aw87390 only has two level of boost voltage
	if index 0 or 1 (volume index > 10) ,the index set to 1
	if index 2 or 3  (volume index <= 10),the index set to 0 */

	ret = aw87xxx_set_boost_voltage(aw87xxx, ctrl_value);
	if (ret < 0) {
		AW_DEV_LOGE(aw87xxx->dev, "set dev_index[%d] boost voltage failed, index: %d,%s",
			aw87xxx->dev_index, ctrl_value, aw87xxx_volume_boost[ctrl_value]);
		return ret;
	}
	AW_DEV_LOGI(aw87xxx->dev, "aw87xxx set volume boost voltage index is %d", ctrl_value);
	return 0;

}

static int aw87xxx_volume_boost_get(struct snd_kcontrol *kcontrol,
                        struct snd_ctl_elem_value *ucontrol)
{
	struct aw87xxx *aw87xxx = (struct aw87xxx *)kcontrol->private_value;
	int level = 0;
	...
	ret = aw87xxx_get_boost_voltage(aw87xxx, &level);
	if (ret < 0) {
		AW_DEV_LOGE(aw87xxx->dev, "get boost voltage failed");
	}
	ucontrol->value.integer.value[0] = level;

	AW_DEV_LOGI(aw87xxx->dev, "aw87xxx get volume boost voltage index is %ld", ucontrol->value.integer.value[0]);
	return 0;
}
#endif
						...
						void *codec)
{
	...
	snprintf(kctl_name[3], AW_NAME_BUF_MAX, "aw87xxx_volume_boost_%d",
			aw87xxx->dev_index);

	aw87xxx_kcontrol[3].name = kctl_name[3];
	aw87xxx_kcontrol[3].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	aw87xxx_kcontrol[3].info = aw87xxx_volume_boost_info;
	aw87xxx_kcontrol[3].get = aw87xxx_volume_boost_get;
	aw87xxx_kcontrol[3].put = aw87xxx_volume_boost_put;
				...
				const struct i2c_device_id *id)
{
	...
	static struct kobj_attribute extpa_info_attr = __ATTR(extpa_info, 0664, extpa_info_show, NULL);
	...
	AW_DEV_LOGE(&client->dev, "i2c addr 0x%x",client->addr);
	...
	/* aw87xx probe success*/
	pa_is_aw = true;
	if (ext_debug_kobj) {
		pr_err("ext_pa info sysfs exist");
	} else {
