...
static int aw87xx_rx_topo_id = AW_RX_DEFAULT_TOPO_ID;
static int aw87xx_rx_port_id = AW_RX_DEFAULT_PORT_ID;
...
extern int aw87xx_send_afe_cal_apr(uint32_t param_id,
	void *buf, int cmd_size, bool write, int port_id);
/*
...
extern void aw87xx_set_port_id(int rx_port_id);
#else
static void aw87xx_set_port_id(int rx_port_id)
{
	return;
}
#endif
...
{
	int ret;
	AW_LOGI("aw87xx enter,port id = 0x%x",aw87xx_rx_port_id);
	ret = afe_get_topology(aw87xx_rx_port_id);
	AW_LOGI("topo_id 0x%x", ret);

	if (ret != aw87xx_rx_topo_id)
				...
				void *data, int data_size)
{
	...
	mutex_lock(&g_dsp_lock);
	aw_check_dsp_ready();
	ret = aw87xx_send_afe_cal_apr(param_id, data,
		data_size, true, aw87xx_rx_port_id);
	mutex_unlock(&g_dsp_lock);
	return ret;
}
				...
				void *data, int data_size)
{
	...
	aw_check_dsp_ready();
	ret = aw87xx_send_afe_cal_apr(param_id, data,
			data_size, false, aw87xx_rx_port_id);
	mutex_unlock(&g_dsp_lock);
	return ret;
}
...
void aw87xxx_device_parse_topo_id_dt(struct aw_device *aw_dev)
{
	...
	ret = of_property_read_u32(aw_dev->dev->of_node, "aw-rx-topo-id", &aw87xx_rx_topo_id);
	if (ret < 0) {
		aw87xx_rx_topo_id = AW_RX_DEFAULT_TOPO_ID;
		AW_DEV_LOGI(aw_dev->dev, "read aw-rx-topo-id failed,use default");
	}

	AW_DEV_LOGI(aw_dev->dev, "rx-topo-id: 0x%x",  aw87xx_rx_topo_id);
}
...
void aw87xxx_device_parse_port_id_dt(struct aw_device *aw_dev)
{
	...
	ret = of_property_read_u32(aw_dev->dev->of_node, "aw-rx-port-id", &aw87xx_rx_port_id);
	if (ret < 0) {
		aw87xx_rx_port_id = AW_RX_DEFAULT_PORT_ID;
		AW_DEV_LOGI(aw_dev->dev, "read aw-rx-port-id failed,use default");
	}

	//aw87xx_set_port_id(aw87xx_rx_port_id);
	AW_DEV_LOGI(aw_dev->dev, "rx-port-id: 0x%x", aw87xx_rx_port_id);
}
